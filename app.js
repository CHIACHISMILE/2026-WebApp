...
// ✅ time as string always, ensure HH:MM
function normTimeStr(v){
  const s = String(v ?? "").trim();
  if (!s) return "";
  const m = s.match(/^(\d{1,2}):(\d{2})/);
  if (!m) return s; // keep raw
...
function setSyncBadge(mode, text){
  ...
  elSync.innerHTML = `<span class="dot ${dotClass}"></span><span class="text-sm" style="color:rgba(7,18,26,.86);font-weight:900;">${escapeHtml(text)}</span>`;
}
...
function openItModal(editId=null){
  ...
  itStart.value = normTimeStr(it.start || "");
  itEnd.value = normTimeStr(it.end || "");
...
function toSheetItinerary(it){
  return {
    ...
    StatTime: normTimeStr(it.start || ""),
    EndTime: normTimeStr(it.end || ""),
...
function saveItinerary(){
  const start = normTimeStr(itStart.value || "");
  const end = normTimeStr(itEnd.value || "");
...
function renderItinerary(){
  ...
  if (!items.length){
    itineraryList.innerHTML = `<div class="card p-4 text-sm" style="color: rgba(7,18,26,.56);">今天還沒有行程。可以按「新增行程」。</div>`;
    return;
  }

  items.forEach(it => {
    const start = normTimeStr(it.start||"");
    const end = normTimeStr(it.end||"");
    ...
    card.className = `card p-4 ${itCardClass(it.category)}`;
      ...
      <div class="flex items-start justify-between gap-3">
        <div class="min-w-0">
          <div class="flex items-center gap-2 flex-wrap">
            ...
            <span class="text-xs font-extrabold" style="color: rgba(7,18,26,.62);">${escapeHtml(time)}</span>
            ${pendingOffline ? `<span class="text-xs pending-badge">待上傳</span>` : ``}
          </div>

          <div class="mt-2 text-base font-semibold leading-snug break-words" style="color: rgba(7,18,26,.92);">
            ${it.title || ""}
          </div>

          ${it.location ? `<div class="mt-2 text-xs break-words" style="color: rgba(7,18,26,.62);">${escapeHtml(it.location)}</div>` : ""}
          ${it.link ? `<div class="mt-2 text-xs"><a href="${escapeHtml(it.link)}" target="_blank" rel="noopener" class="link-soft">開啟連結</a></div>` : ""}
          ${it.note ? `<div class="mt-2 text-sm break-words" style="color: rgba(7,18,26,.78);">${it.note}</div>` : ""}
...
// ✅ Pie palette per category (more diverse, stable)
const PIE_PALETTE = [
  "rgba(127,191,232,.88)", // ice
  "rgba(90,167,133,.86)",  // moss
  "rgba(214,161,90,.86)",  // amber
  "rgba(122,120,211,.82)", // lav
  "rgba(207,126,149,.82)", // rose
  "rgba(99,119,137,.78)",  // slate
  "rgba(153,201,240,.86)",
  "rgba(124,194,172,.86)",
  "rgba(235,190,120,.86)",
  "rgba(165,163,230,.84)",
...
function renderAnalysis(){
  ...
  // ✅ Budget UI: remain big, spent small, bar, mom line small
  ...
  budgetRemain.style.color = "rgba(7,18,26,.92)";
  budgetRemain.style.fontWeight = "900";

  budgetSpent.textContent = `已支出 NT$ ${fmtMoney(jqTyTotal)}`;
  budgetSpent.style.color = "rgba(7,18,26,.62)";
  ...
  momLine.style.color = "rgba(7,18,26,.56)";
  momLine.style.fontWeight = "900";

  // ✅ Pie: by *category* (早餐/午餐/.../交通（機票）/住宿...)
  const catMap = new Map();
  for (const e of filtered){
    const twd = toTwd(e, fx);
    const cat = String(e.category || "其他");
    catMap.set(cat, (catMap.get(cat) || 0) + twd);
  }
  ...
  if (!items.length){
    analysisExpenseList.innerHTML = `<div class="card p-4 text-sm" style="color: rgba(7,18,26,.56);">此篩選條件下沒有消費。</div>`;
  } else {
    items.forEach(e => {
      const twd = toTwd(e, fx);
      const originalLine = (e.currency && e.currency !== "TWD")
        ? `<div class="text-xs font-extrabold" style="color: rgba(7,18,26,.56);">${escapeHtml(e.currency)} ${fmtMoney(e.amount)}（原幣）</div>`
        : ``;
      ...
      card.innerHTML = `
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0">
            <div class="flex items-center gap-2 flex-wrap">
              <span class="tag tag-exp" data-cat="${escapeHtml(e.category)}">${escapeHtml(e.category)}</span>
              <div class="text-base font-semibold break-words" style="color: rgba(7,18,26,.92);">${escapeHtml(e.item)}</div>
              ${pendingOffline ? `<span class="text-xs pending-badge">待上傳</span>` : ``}
            </div>

            <div class="mt-1 text-sm font-extrabold" style="color: rgba(7,18,26,.60);">
              ${escapeHtml(e.payer)} • ${escapeHtml(e.location)} • ${escapeHtml(e.payment)} • ${escapeHtml(e.date)}
            </div>

            <div class="mt-2 text-2xl font-extrabold" style="color: rgba(7,18,26,.92);">NT$ ${fmtMoney(twd)}</div>
            ${originalLine}

            ${e.note ? `<div class="mt-2 text-sm break-words" style="color: rgba(7,18,26,.76);">${escapeHtml(e.note)}</div>` : ""}
            <div class="mt-2 text-xs font-extrabold" style="color: rgba(7,18,26,.56);">分攤：${escapeHtml(splits)}</div>
          </div>
...
function renderFilterChips(){
  ...
  if (!chips.length){
    filterChips.innerHTML = `<span class="text-xs font-extrabold" style="color: rgba(7,18,26,.56);">未套用篩選</span>`;
    return;
  }
  ...
  wrap.innerHTML = `
    <div class="modal-sheet">
      <div class="modal-head">
        <div class="text-base font-extrabold" style="color: rgba(7,18,26,.92);">匯率設定（固定全旅程）</div>
        <button class="btn btn-ghost" id="btnFxClose">關閉</button>
      </div>
      <div class="modal-body">
        <div class="text-xs font-extrabold" style="color: rgba(7,18,26,.56);">輸入「1 外幣 = ? TWD」</div>
        <div class="grid grid-cols-2 gap-3 mt-4">
          ${fxField("NOK")}${fxField("ISK")}${fxField("EUR")}${fxField("GBP")}${fxField("AED")}
        </div>
      </div>
...
// ---------- Expense Edit modal (fix select missing + involved buttons) ----------
let expEditModalEl = null;
let editingExpenseId = null;
let editingInvolvedSet = new Set();
...
function buildExpenseEditModal(){
  ...
  wrap.innerHTML = `
    <div class="modal-sheet">
      <div class="modal-head">
        <div class="text-base font-extrabold" style="color: rgba(7,18,26,.92);">編輯消費明細</div>
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <div class="col-span-2">
            <div class="flex items-center justify-between">
              ...
              <button class="btn btn-ghost text-sm" id="btnExpSplitAll">全選</button>
            </div>
            <div id="expEInvolvedBtns" class="mt-2 flex flex-wrap gap-2"></div>
          </div>
        </div>
      </div>
  ...
  wrap.querySelector("#btnExpSplitAll").addEventListener("click", () => {
    const all = PEOPLE;
    const shouldAll = editingInvolvedSet.size !== all.length;
    editingInvolvedSet = new Set(shouldAll ? all : []);
    renderEditInvolvedButtons();
  });

  wrap.addEventListener("click", (e) => { if (e.target === wrap) closeExpenseEditModal(); });

  // when payer changes, default add payer into involved if empty
  wrap.querySelector("#expEWho").addEventListener("change", () => {
    const payer = wrap.querySelector("#expEWho").value;
    if (editingInvolvedSet.size === 0) editingInvolvedSet.add(payer);
    renderEditInvolvedButtons();
  });
...
function fillExpenseEditModal(e){
  ...
  const inv = String(e.involved||"").split(",").map(s=>s.trim()).filter(Boolean);
  editingInvolvedSet = new Set(inv.length ? inv : [e.payer]);
  renderEditInvolvedButtons();
}
function renderEditInvolvedButtons(){
  const box = expEditModalEl.querySelector("#expEInvolvedBtns");
  box.innerHTML = "";
  PEOPLE.forEach(p => {
    const b = document.createElement("button");
    b.className = "tag";
    b.textContent = p;
    b.classList.toggle("active", editingInvolvedSet.has(p));
    b.addEventListener("click", () => {
      if (editingInvolvedSet.has(p)) editingInvolvedSet.delete(p);
      else editingInvolvedSet.add(p);
      b.classList.toggle("active", editingInvolvedSet.has(p));
    });
    box.appendChild(b);
  });
...
function saveExpenseEditFromModal(){
  ...
  const involved = Array.from(editingInvolvedSet).join(",");

  if (!item) return alert("請輸入消費名稱");
  if (!Number.isFinite(amount) || amount <= 0) return alert("請輸入正確金額");
  if (!involved) return alert("請至少選一位分攤人員");

  const all = load(LS.expenses, []);
  const idx = all.findIndex(x => x.id === id);
  if (idx < 0) return;

  all[idx] = { ...all[idx], payer:who, location:where, category, payment:pay, currency, amount, item, note, involved };
  save(LS.expenses, all);
  ...
  for (const item of rows){
    ...
    if (table === "Itinerary"){
      it.push({
        id: String(r.ID || ""),
        date: normalizeDateKey(r.Date || ""),
        start: normTimeStr(r.StatTime || ""),  // ✅ StatTime (per sheet)
        end: normTimeStr(r.EndTime || ""),
        category: String(r.Category || ""),
...
function setupPullToRefresh(containers){
  ...
  if (!ind){
    ...
    ind.style.color = "rgba(7,18,26,.76)";
    ...
    ind.style.transform = "translateY(-140%)";
  ...
  containers.forEach(el => {
    ...
    el.addEventListener("touchmove", (e) => {
      ...
      ind.style.transform = `translateY(${(-140 + pct*140)}%)`;
    }, { passive:false });
      ...
      if (dist >= THRESH){
        ...
        try{
          if (!navigator.onLine){
            ...
            setTimeout(()=>ind.style.transform="translateY(-140%)", 800);
            return;
          }
          await pullAll();
          renderAll();
          ind.textContent = "已更新";
          setTimeout(()=>ind.style.transform="translateY(-140%)", 500);
        }catch{
...
function attachEvents(){
  ...
  btnClearFilter?.addEventListener("click", () => { state.filter.who.clear();state.filter.where.clear();state.filter.category.clear();state.filter.pay.clear(); buildFilterModal(); renderAnalysis(); });
...
ㄌ  return new Date(y, m-1, d, 12, 0, 0);
}
function fmtDate(d){
  const y = d.getFullYear();
  const m = String(d.getMonth()+1).padStart(2,"0");
